...
def line_segment_fitting(point_sequences, 
    ...
    for i in range(0, len(point_sequences)):
        ...
        segment_lengths.append(np.sqrt((p2[0]-p1[0])**2 + (p2[1]-p1[1])**2))
    ...
    return fitted_segments, segment_from, segment_lengths, fitting_errors
...
def main():
    ...
    point_sequences, sequence_from = \
                        douglas_peucker_track_segmentation.dp_segmentation(tracks, 5)
    print "There are %d sequences."%len(point_sequences)

    # Line segment fitting
    fitted_segments, segment_from, segment_lengths, fitting_errors = \
    ...
    bad_lines = []
    bad_colors = []
    for seg_idx in range(0, len(fitted_segments)):
        seg = fitted_segments[seg_idx]
        if fitting_errors[seg_idx] <= ERROR_THRESHOLD and \
                    segment_lengths[seg_idx] >= LENGTH_THRESHOLD:
            good_lines.append([(seg[0], seg[1]),(seg[2], seg[3])])
            good_colors.append(color_strings[(len(good_lines)-1)%6])
        else:
            bad_lines.append([(seg[0], seg[1]),(seg[2], seg[3])])
            bad_colors.append(color_strings[(len(bad_colors)-1)%6])
    print "\t%d good lines"%len(good_lines)
    print "\t%d bad lines"%len(bad_lines)
    good_collection = LineCollection(good_lines, colors=good_colors, linewidths=3)
    bad_collection = LineCollection(bad_lines, colors=bad_colors, linewidths=3)
    ax1.add_collection(good_collection) 
    ax2.add_collection(bad_collection)
