...
def main():
    ...
    parser.add_option("-s", "--samplepoint", dest="sample_point", help="Sample point file to save.", type="string", metavar="SamplePointFile")
    parser.add_option("-l", "--lines", dest="extracted_lines", help="Generated line segment.")
    ...
    if not options.sample_point:
        parser.error("Output sample point filename not found!")
    if not options.extracted_lines:
        parser.error("Output extracted line file not specified.")
    ...
    lines = extract_line_segments(img,
                                  ...
                                  SEARCH_RANGE,
                                  P_REMOVAL)
    print "%d line segments are extracted."%len(lines)
    with open(options.extracted_lines, "wb") as fout:
        cPickle.dump(lines, fout, protocol=2)

    visualize_extracted_lines(point_cloud, lines, LOC, R)

    # Filter GPS point with grid to generate sample
    sample_point_cloud = filter_point_cloud_using_grid(point_cloud, 
                                                       10,
                                                       LOC,
                                                       R)
    visualize_sample_point_cloud(sample_point_cloud, LOC, R)

    # Correct sample direction with extracted line segments.
    new_sample_point_cloud = correct_direction_with_lines(sample_point_cloud,
                                                          lines)
    print "There are %d sample points"%new_sample_point_cloud.locations.shape[0]

    visualize_sample_point_cloud(new_sample_point_cloud, LOC, R)

    with open(options.sample_point, 'wb') as fout:
