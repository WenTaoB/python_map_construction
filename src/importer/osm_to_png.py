...
def main():
    parser = OptionParser()
    parser.add_option("-m", "--osm", dest="osm_data", help="Input open street map data", metavar="OSM_DATA", type="string")
    parser.add_option("-o", "--output_img", dest="output_img", help="Output image file name", metavar="OUTPUT", type="string")
    ...
    if not options.osm_data:
        parser.error("Input osm_data file not found!")
    if not options.output_img:
        parser.error("Output image file not specified!")
